...
class ChatCubit extends Cubit<ChatState> {
  ...
  Future<void> fetchMessages(int conversationId) async {
    ...
    try {
      final res =
          await _repositoriesImp.getConversationMessages(conversationId);
      if (res.isError || res.data == null) {
        emit(ConversationError(res.message ?? 'Failed to fetch messages'));
        return;
      }
      emit(ConversationLoaded(res.data!));
    } catch (e) {
  ...
  Future<void> sendMessage(int receiverId, String content) async {
    try {
      ...
      if (state is ConversationLoaded) {
        final current =
            List<Message>.from((state as ConversationLoaded).messages);
        ...
        emit(SendMessageLoaded(current));
      }
    } catch (e) {
      emit(ConversationError(e.toString()));
    }
